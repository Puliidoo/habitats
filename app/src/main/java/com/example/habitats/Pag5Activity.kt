...
class Pag5Activity : AppCompatActivity() {

    private var puntos: Int = 0
    private lateinit var marcador: TextView
    private var puntosSumados = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageView>(R.id.avatar3).setOnClickListener {
            ...
            if (!puntosSumados) {
                ...
                val avatarSeleccionado = prefs.getInt("avatarSeleccionado", R.drawable.avatar1)
                val registro = "$nombreUsuario|$avatarSeleccionado|$puntos"

                val registros = prefs.getStringSet("registros", mutableSetOf())!!.toMutableSet()
                registros.add(registro)
                prefs.edit().putStringSet("registros", registros).apply()
